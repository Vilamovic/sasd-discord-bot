...
export const plus = {
  data: new SlashCommandBuilder()
    .setName('plus')
    .setDescription('[CS+] Nadaj plus użytkownikowi')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Update plus_count
    await supabase.from('users').update({ plus_count: (portalUser.plus_count || 0) + 1 }).eq('id', portalUser.id);

    // Emit Discord event (for log channel)
